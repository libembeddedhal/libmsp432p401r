...
class SystemController final : public sjsu::SystemController
{
 public:
  ...
  {
    if (peripheral.device_id >= static_cast<int>(kClockPeripheralCount))
    {
      return 0_MHz;
    }
 ...
 private:
  ...
  {
    if (Value(clock) > Value(Clock::kBackup))
    {
  ...
  ///
  /// @note When selecting the oscillator source for either SMCLK or SMCLK,
  /// the
  ...
  void SetClockSource(Clock clock, Oscillator source) const
  {
    ...
    switch (clock)
    {
      ...
      {
        if (select_value > Value(Oscillator::kReference))
        {
          throw sjsu::Exception(
              std::errc::invalid_argument,
      ...
      case Clock::kBackup:
      {
        switch (source)
        {
          ...
          default:
            ...
            throw sjsu::Exception(std::errc::invalid_argument,
  ...
  units::frequency::hertz_t ConfigureDcoClock() const
  {
    ...
    if (dco_config.enabled)
    {
      ...
      if ((kMinimumDcoFrequency > kTargetFrequency) &&
      ...
      // =======================================================================
      // Step 3. Calculate the signed 10-bit tuning value using Equation 6
      // from
